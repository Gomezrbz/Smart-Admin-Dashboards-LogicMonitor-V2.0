...
**Generated for:** SmartAdmin Connected Experience redesign v2  
**Santaba release:** 242  
...
| `Basement/SmartAdmin Dashboards.json` | SmartAdmin Cloud/Local - License Counts | License consumption | Admins, capacity/FinOps | bigNumber×14 | `accountname=proservices` only | Account name | LicenseCounts | Overview | Replace hardcoded account with placeholder; detail on coverage | **04 Coverage, Capacity & Licenses** (detail); **01** (summary) |
| `Basement/SmartAdmin Dashboards.json` | Collector Health | Collector performance | Platform engineers | text×3, bigNumber×11, cgraph×13, dynamicTable×3, alert | `defaultResourceGroup=*`, `defaultResourceName=*` | Resource / group | Collector JVM, DataCollectingTasks, ActiveDiscoveryTasks | Overview alerts | Keep as canonical L3 | **07 Collector Health** |
...
**Disposition:** Do **not** clone the full pack. Document as Level-3 relationship targets with `{{PORTAL_BASE}}` placeholders. Prefer importing OOTB packages separately, then wiring links post-import.
